...
function extract_generic_propagation_times {
  # Extract lines containing GenericPropagation execution times.
  # sed out unique module name and milliseconds spent executing.
  # example input:
  #     "|15:58:00.819|    (INFO) [R:GenericPropagation:detector3] Finished execution in 105.012ms"
  # output:
  #     "GenericPropagation:detector3 105.012"
  pattern="s/^.*\[R:\(\S*\)\].*in\s*\(\S*\)ms.*$/\1\ \2/p"
  readarray -t entries <<< $(grep -e 'GenericPropagation:.* Finished execution' "$log" | sed -n $pattern)

  # Now, collect total run-time for unique modules
  declare -A module_runtimes
  for entry in "${entries[@]}"; do
    local module=$(echo $entry | awk '{print $1}')
    local milliseconds=$(echo $entry | awk '{print $2}')

    module_runtimes[$module]=$(_bc "${module_runtimes[$module]:-0} + $milliseconds")
  done

  # Print out all modules and their run-times in seconds
  for key in "${!module_runtimes[@]}"; do
    echo $key $(_bc "${module_runtimes[$key]}" / 1000) # for seconds
  done
}
...
if [ $module_flag == 'true' ]; then
  ...
  extract_generic_propagation_times
fi
